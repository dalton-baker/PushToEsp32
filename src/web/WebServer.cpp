...
bool PushToWebServer::isRunning() {
    return _running;
}

String PushToWebServer::getAPIP() {
    return apIP;
}

String PushToWebServer::getSSID() {
    return AP_SSID;
}

String PushToWebServer::getPassword() {
    return AP_PASSWORD;
}

void PushToWebServer::setupRoutes() {
    // Single-page UI
    ...
    // API: Get diagnostics (text dump, useful for debugging via curl)
...
void PushToWebServer::handleGetConfig(AsyncWebServerRequest* request) {
...
void PushToWebServer::handleConfigSubmit(AsyncWebServerRequest* request) {
...
void PushToWebServer::handleAlignmentSubmit(AsyncWebServerRequest* request) {
    ...
    if (request->hasParam("star", true) && request->hasParam("ra", true) && request->hasParam("dec", true)) {
        ...
        if (!pos.valid) {
            request->send(503, "text/plain", "Sensors not ready");
            return;
        }

        // Save raw measured az/alt and the catalog RA/Dec.
...
void PushToWebServer::handleGetStatus(AsyncWebServerRequest* request) {
...
void PushToWebServer::handleGetPosition(AsyncWebServerRequest* request) {
    StaticJsonDocument<256> doc;
...
void PushToWebServer::handleGetDiagnostics(AsyncWebServerRequest* request) {
...
void PushToWebServer::onWebSocketEvent(AsyncWebSocket* server, AsyncWebSocketClient* client,
                                       AwsEventType type, void* arg, uint8_t* data, size_t len) {
    switch(type) {
        case WS_EVT_CONNECT:
...
void PushToWebServer::broadcastPosition() {
    if (!ws || ws->count() == 0) {
...
void PushToWebServer::handleSetTime(AsyncWebServerRequest* request) {
...
void PushToWebServer::handleFinalize(AsyncWebServerRequest* request) {
...
void PushToWebServer::handleGetAlignmentStars(AsyncWebServerRequest* request) {
    StaticJsonDocument<512> doc;

    AlignmentStar star0 = _config->getAlignmentStar(0);
    AlignmentStar star1 = _config->getAlignmentStar(1);

    AlignmentStar stars[2] = { star0, star1 };
    for (int i = 0; i < 2; i++) {
        JsonObject starObj = doc.createNestedObject("star" + String(i + 1));
        starObj["ra"] = stars[i].ra;
        starObj["dec"] = stars[i].dec;
        starObj["az"] = stars[i].az;
        starObj["alt"] = stars[i].alt;
        starObj["name"] = _config->getAlignmentStarName(i);
        starObj["configured"] = stars[i].valid;
    }

    doc["aligned"] = _config->isAligned();

    // Indicate which correction model is active
    if (star0.valid && star1.valid) {
        doc["alignMode"] = "2star";
    } else if (star0.valid || star1.valid) {
        doc["alignMode"] = "1star";
    } else {
        doc["alignMode"] = "none";
    }
...
bool PushToWebServer::hasWebSocketClients() {
